...
public class FileUploadService : IFileUploadService
{
    ...
    private readonly string[] _allowedTaskExtensions = { ".pdf", ".docx", ".zip", ".rar" };
    private const long MaxCvFileSize = 5 * 1024 * 1024; // 5 MB
    private const long MaxImageFileSize = 2 * 1024 * 1024; // 2 MB
    private const long MaxTaskFileSize = 10 * 1024 * 1024; // 10 MB
    ...
    public FileUploadService(IWebHostEnvironment environment, ILogger<FileUploadService> logger)
    {
        ...
        Directory.CreateDirectory(Path.Combine(_uploadPath, "programs"));
        Directory.CreateDirectory(Path.Combine(_uploadPath, "tasks"));
    }
    ...
    public Task<ApiResponse<bool>> ValidateFileAsync(IFormFile file, string fileType)
    {
        ...
        else if (fileType == "profile-picture" || fileType == "program-image") 
        {
        ...
        {
            if (!_allowedTaskExtensions.Contains(extension))
            {
                return Task.FromResult(ApiResponse<bool>.ErrorResponse(
                    $"Invalid file type. Allowed: {string.Join(", ", _allowedTaskExtensions)}"
                ));
            }

            if (file.Length > MaxTaskFileSize)
            {
                return Task.FromResult(ApiResponse<bool>.ErrorResponse(
                    "Task file size must not exceed 10 MB"
                ));
            }
        }
        else
        {
            return Task.FromResult(ApiResponse<bool>.ErrorResponse(
                "Invalid file type. Must be 'cv', 'profile-picture', 'program-image' or 'task'"
            ));
        }
