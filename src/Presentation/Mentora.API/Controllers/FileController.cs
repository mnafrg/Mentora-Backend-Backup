...
public class FileController : ControllerBase
{
    ...
    [HttpPost("upload-program-image")]
    public async Task<IActionResult> UploadProgramImage(IFormFile file)
    {
        var userId = User.GetUserId();

        if (userId == Guid.Empty)
        {
            return Unauthorized(new { message = "You must be logged in to upload program images." });
        }

        var result = await _fileUploadService.UploadFileAsync(file, "program-image", userId);
    ...
    public async Task<IActionResult> UploadTaskAttachment(IFormFile file)
    {
        var userId = User.GetUserId();

        if (userId == Guid.Empty)
        {
            return Unauthorized(new { message = "You must be logged in as a mentor to upload task attachments." });
        }

        var result = await _fileUploadService.UploadFileAsync(file, "task", userId);
