...
public class PostController : ControllerBase
{
    ...
    // GET /api/communities/{communityId}/posts?page=1&pageSize=10
    [HttpGet("{communityId:guid}/posts")]
    public async Task<IActionResult> GetAllPostsByCommunity(
        Guid communityId,
        [FromQuery] PaginationParamsDto pagination)
    {
        var currentUserId = User.GetUserId();

        var result = await _postService.GetAllPostsByCommunityAsync(
            communityId,
            pagination,
            currentUserId);
    ...
    // GET /api/communities/posts/feed?page=1&pageSize=10
    [HttpGet("posts/feed")]
