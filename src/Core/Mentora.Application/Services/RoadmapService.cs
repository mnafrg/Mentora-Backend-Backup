...
namespace Mentora.Application.Services
{
    public class RoadmapService : IRoadmapService
    {
        ...
        public async Task<ApiResponse<bool>> UpdateRoadmapAsync(int roadmapId, UpdateRoadmapDto dto, Guid mentorId)
        {
            ...
            var incomingPhaseIds = dto.Phases.Where(p => p.PhaseId.HasValue).Select(p => p.PhaseId.Value).ToList();
            bool isDeletingPhases = roadmap.Phases.Any(p => !incomingPhaseIds.Contains(p.RoadmapPhaseId));
            bool isDeletingTopics = false;

            foreach (var phaseDto in dto.Phases.Where(p => p.PhaseId.HasValue))
            {
                var existingPhase = roadmap.Phases.FirstOrDefault(p => p.RoadmapPhaseId == phaseDto.PhaseId);
                if (existingPhase != null)
                {
                    var incomingTopicIds = phaseDto.Topics.Where(t => t.Id.HasValue).Select(t => t.Id.Value).ToList();
                    if (existingPhase.Topics.Any(t => !incomingTopicIds.Contains(t.TopicId)))
                    {
                        isDeletingTopics = true;
                        break;
                    }
                }
            }
            ...
            roadmap.TargetLevelFrom = dto.TargetLevelFrom;
            roadmap.TargetLevelTo = dto.TargetLevelTo;
            ...
            var phasesToRemove = roadmap.Phases.Where(p => !incomingPhaseIds.Contains(p.RoadmapPhaseId)).ToList();
            foreach (var phase in phasesToRemove) roadmap.Phases.Remove(phase);
            ...
            foreach (var phaseDto in dto.Phases)
            {
                ...
                {
                    existingPhase.Title = phaseDto.Title;
                    existingPhase.Summary = phaseDto.Summary;

                    var currentIncomingTopicIds = phaseDto.Topics.Where(t => t.Id.HasValue).Select(t => t.Id.Value).ToList();
                    var topicsToRemove = existingPhase.Topics.Where(t => !currentIncomingTopicIds.Contains(t.TopicId)).ToList();
                    foreach (var topic in topicsToRemove) existingPhase.Topics.Remove(topic);

                    foreach (var topicDto in phaseDto.Topics)
                    {
                        var existingTopic = existingPhase.Topics.FirstOrDefault(t => t.TopicId == topicDto.Id);
                        if (existingTopic is not null)
                        {
                            existingTopic.Title = topicDto.Title;
                            existingTopic.Summary = topicDto.Summary;

                        
                            var incomingMatIds = topicDto.Materials.Where(m => m.Id.HasValue).Select(m => m.Id.Value).ToList();
                            var matsToRemove = existingTopic.Materials.Where(m => !incomingMatIds.Contains(m.MaterialId)).ToList();
                            foreach (var m in matsToRemove) existingTopic.Materials.Remove(m);

                            foreach (var matDto in topicDto.Materials)
                            {
                                var existingMat = existingTopic.Materials.FirstOrDefault(m => m.MaterialId == matDto.Id);
                                if (existingMat != null)
                                {
                                    existingMat.Title = matDto.Title; existingMat.Url = matDto.Url; existingMat.MaterialType = matDto.MaterialType;
                                }
                                else
                                {
                                    existingTopic.Materials.Add(new TopicMaterial { Title = matDto.Title, Url = matDto.Url, MaterialType = matDto.MaterialType });
                                }
                            }
                            ...
                            {
                                var incomingTaskIds = topicDto.Tasks.Where(t => t.Id.HasValue).Select(t => t.Id.Value).ToList();
                                var tasksToRemove = existingTopic.Tasks.Where(t => !incomingTaskIds.Contains(t.TaskId)).ToList();
                                foreach (var task in tasksToRemove) existingTopic.Tasks.Remove(task);
                                ...
                                foreach (var taskDto in topicDto.Tasks)
                                {
                                    ...
                                    {
                                        existingTopic.Tasks.Add(new TopicTask { Title = taskDto.Title, Description = taskDto.Description, DeadLine = taskDto.DeadLine, TaskAttachmentUrl = taskDto.AttachmentUrl });
                                    }
                                }
                            }
                        }
                        ...
                        {
                            existingPhase.Topics.Add(new Topic { Title = topicDto.Title, Summary = topicDto.Summary });
                        }
                    }
                }
                else
                {
                    roadmap.Phases.Add(new RoadmapPhase { Title = phaseDto.Title, Summary = phaseDto.Summary });
                }
            }
