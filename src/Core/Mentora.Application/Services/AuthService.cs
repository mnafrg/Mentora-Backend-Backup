    ...
    public class AuthService : IAuthService
    {
        ...
        CompleteMenteeProfileRequest request)
        {
            ...
            try
            {
                ...
                if (!Enum.TryParse<CurrentLevel>(request.CurrentLevel, true, out var currentLevel))
                {
                    return ApiResponse<RegistrationCompleteResponse>.ErrorResponse($"Invalid current level: {request.CurrentLevel}");
                }
                ...
                if (request.TechnologyInterests.Count < 1 || request.TechnologyInterests.Count > 5)
                {
                    return ApiResponse<RegistrationCompleteResponse>.ErrorResponse("Please select between 1 and 5 tools");
                }

        
                // Validate and add each technology with its experience level
                foreach (var techInterest in request.TechnologyInterests)
                {
                    // Parse the experience level for this specific technology
                    if (!Enum.TryParse<ExperienceLevel>(techInterest.ExperienceLevel, true, out var techExperienceLevel))
                    {
                        return ApiResponse<RegistrationCompleteResponse>.ErrorResponse(
                            $"Invalid experience level '{techInterest.ExperienceLevel}' for technology {techInterest.TechnologyId}. " +
                            "Valid values are: None, Beginner, Intermediate, Advanced"
                        );
                    }
                    ...
                    {
                        UserId = session.UserId,
                        TechnologyId = techInterest.TechnologyId,
                        ExperienceLevel = techExperienceLevel  // Use the specific experience level for this technology
                    };
        ...
        public async Task<ApiResponse<AuthResponse>> LoginAsync(LoginRequest request)
        {
            ...
            if (user.Role.ToString() == "Mentor")
            {
                var mentorProfile = await _unitOfWork.MentorProfiles.GetByUserIdAsync(user.UserId);
                profileId = mentorProfile?.UserId;
            }
            else if (user.Role.ToString() == "Mentee")
            {
                var menteeProfile = await _unitOfWork.MenteeProfiles.GetByUserIdAsync(user.UserId);
                profileId = menteeProfile?.UserId;
            }
